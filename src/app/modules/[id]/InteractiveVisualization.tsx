...
// ==========================================
// MOD-25: Audio Spectrogram Visualizer
// ==========================================
function AudioSpectrogramVisualizer({ description }: { description: string }) {
    const [isPlaying, setIsPlaying] = useState(false);
    ...
    useEffect(() => {
        ...
        if (isPlaying) {
            interval = setInterval(() => setT(prev => prev + 1), 100);
        }
        return () => clearInterval(interval);
    }, [isPlaying]);

    // Generate pseudo-spectrogram data based on time
    const freqs = Array.from({ length: 24 }, (_, i) => {
        const base = 20 + Math.sin(i * 0.5 + t * 0.2) * 15;
        const noise = Math.random() * 10;
        return isPlaying ? Math.max(10, base + noise) : 10;
    });

    return (
        <div className="glass-card p-8 border-sky-500/20 bg-gradient-to-br from-sky-900/20 to-transparent">
            <h3 className="text-2xl font-bold text-sky-400 mb-4 flex items-center">
                <Activity className="mr-3 w-6 h-6" /> Spektrogram Suara Interaktif
            </h3>
            <p className="text-gray-300 mb-6">{description}</p>
            ...
            <div className="flex flex-col gap-6">
                <div className="flex justify-center">
                    <button
                        ...
                        className={`flex items-center px-6 py-2 rounded-full font-bold transition-colors ${isPlaying ? 'bg-red-500/20 text-red-400 border border-red-500/50 hover:bg-red-500/30' : 'bg-sky-500 hover:bg-sky-600 text-white shadow-[0_0_15px_#0ea5e9]'}`}
                    >
                        {isPlaying ? <span className="w-4 h-4 rounded-sm bg-red-400 mr-2 animate-pulse" /> : <Play className="w-4 h-4 fill-white mr-2" />}
                        {isPlaying ? 'Hentikan Perekaman' : 'Mulai Bicara (Simulasi)'}
                    </button>
                </div>

                <div className="bg-black/50 p-6 rounded-xl border border-white/10 h-48 flex items-end justify-center gap-1 overflow-hidden">
                    {freqs.map((h, i) => (
                        <div key={i} className="w-full max-w-[12px] rounded-t-sm transition-all duration-100 ease-in-out flex flex-col justify-end gap-1" style={{ height: '100%' }}>
                            {/* Color mapping based on height "intensity" */}
                            <div className={`w-full transition-all duration-100 ${h > 35 ? 'bg-red-500' : h > 25 ? 'bg-yellow-500' : h > 15 ? 'bg-sky-400' : 'bg-sky-900/50'}`} style={{ height: `${h}%` }} />
                        </div>
                    ))}
                </div>

                <div className="bg-sky-900/20 p-4 rounded text-sm text-center border border-sky-500/20">
                    <strong className="text-sky-300">Bagaimana AI Mendengar?</strong> AI tidak mendengar "Halo", melainkan membaca gelombang frekuensi (Spektrogram) seperti gambar di atas, lalu menebak huruf/kata dari pola frekuensi tersebut!
                </div>
            </div>
        </div>
    );
